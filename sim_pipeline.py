...
def opt(i_sim):
    # Admixture
    file_admixture = 'admix{:0>2d}.txt'.format(i_sim + 1)
    ...
    pop_admix, pop_sources_names = read_admixture(path_data_in + file_admixture)
    ...
    opt = opt_wnd(pop_names=pop_names,
                  pop_admix=pop_admix,
                  ...
                  path_data_out=path_data_out + 'estim{:0>2d}'.format(i_sim + 1),
                  pref_wnd_init='d{:0>2d}'.format(i_sim + 1), alpha=1)
...
# Initial Tree
file_tree = 'sim_tree.nwk'


n_sim = 50  # number of windows
n_thr = 30
with Pool(n_thr) as workers:
    pmap = workers.map
    pmap(opt, range(n_sim))
